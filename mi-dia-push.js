...
async function updateButton(){const b=document.getElementById('pushBtn');if(!b)return;let on=false;try{const reg=await navigator.serviceWorker.ready;on=Notification.permission==='granted'&&!!(await reg.pushManager.getSubscription())}catch(_){}b.textContent=on?'🔔 Avisos activos':'🔔 Activar avisos';}
function addButton(){if(document.getElementById('pushBtn'))return;const bar=document.querySelector('.bar');if(!bar)return;const b=document.createElement('button');b.id='pushBtn';b.className='btn secondary';b.textContent='🔔 Activar avisos';b.onclick=enable;bar.insertBefore(b,bar.lastElementChild);setTimeout(updateButton,500)}
const oldBoot=window.boot;window.boot=async function(){await oldBoot();setTimeout(addButton,200)};
setTimeout(()=>{if(window.user)addButton()},1000);
})();
